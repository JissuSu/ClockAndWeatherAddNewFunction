...
public class SettingActivity extends ListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		try {
			...
			tt.start();
			DataStore.setData(context, DataStore.CITY_NAME, Preferences
					.getLocatedCity(context, appWidgetId).toLowerCase().replace("'", ""));
			Intent update = new Intent(context, XKDownLoadUtil.class).putExtra(
					"ifLoad", true);
			startService(update);
